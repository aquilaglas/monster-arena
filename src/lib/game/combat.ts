...
export async function saveCombatResult(
  ...
  await sql`
    INSERT INTO combat_history
      (player_id, player_monster_id, arena_level, opponent_type_id, won, money_earned, experience_gained)
    VALUES
      (${playerId}, ${playerMonsterId}, ${arenaLevel}, ${opponentTypeId}, ${won}, ${moneyEarned}, ${experienceGained})
  `;

  if (won) {
    await sql`
      UPDATE player_monsters
      SET hp = max_hp, experience = experience + ${experienceGained}, updated_at = NOW()
      WHERE id = ${playerMonsterId}
    `;
  } else {
    await sql`
      UPDATE player_monsters
      SET hp = max_hp, updated_at = NOW()
      WHERE id = ${playerMonsterId}
    `;
  }
}
